...
// Hardware: ESP32 + DHT11 (GPIO17) + Soil ADC (GPIO36) + pH ADC (GPIO39) + Ultrasonic (TRIG19/ECHO18) + Pump via Motor2 + SH1107 OLED
// Firebase Paths:
...
float readDistanceCm()
{
  ...
  digitalWrite(PIN_US_TRIG, LOW);
  long dur = pulseIn(PIN_US_ECHO, HIGH, 30000UL);
  if (dur == 0)
    return NAN;
...
int tankPercent(float distanceCm)
{
  if (isnan(distanceCm))
    return -1;
  float waterCol = TANK_HEIGHT_CM - distanceCm;
  if (waterCol < 0)
    waterCol = 0;
  if (waterCol > TANK_HEIGHT_CM)
    waterCol = TANK_HEIGHT_CM;
  return (int)roundf((waterCol / TANK_HEIGHT_CM) * 100.0f);
}
...
void loop()
{
  ...
  if (now - lastSensorRead >= INTERVAL_SENSORS)
  {
    ...
    S.tankCm = readDistanceCm();
    S.tankPct = tankPercent(S.tankCm);
